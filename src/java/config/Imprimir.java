...
public class Imprimir {
    ...
    public static void imprimirDocumento(List<Venta> lista, String serie,Cliente c,Empleado user) {
        ...
        printer.printTextWrap(2, 2, 0, 31, "DIRECCION:Ciudad de El Alto Villa Esperanza");
        int longitudName = c.getNom().length();
        String texto = "CLI:" + (longitudName > 14 ? c.getNom().substring(0, 14) : c.getNom()) + " CI:" + (c.getDni().length() > 8 ? c.getDni().substring(0, 8) : c.getDni());
        ...
        printer.printTextWrap(rowCount + 13, rowCount + 13, 1, 31, "SubTotal  :Bs." + dfoFormat.format(total-(total*IGV)));
        printer.printTextWrap(rowCount + 14, rowCount + 14, 1, 31, "IGV       :Bs." + dfoFormat.format(total*IGV));
        printer.printTextWrap(rowCount + 15, rowCount + 16, 1, 31, "Total     :Bs." + total);
